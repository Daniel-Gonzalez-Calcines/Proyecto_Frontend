...
function MainStats() {
    ...
    return (
        <>
            <Menu />
            <br />
            <Button color={showData === 0 ? 'warning' : 'error'} variant="contained" onClick={() => changeShowData(0, 0)}>
                Lista de jugadores
            </Button>
            {jsonData?.sesion.sessions.map((session, index) => (
                <Grid2 container spacing={2} style={{ width: '100vw', margin: '10 auto' }} key={index}>
                    ...
                    <Grid2 size={3}>
                        <Button color={showData === 1 && showSession === index ? 'warning' : 'error'}  variant='contained' fullWidth onClick={() => changeShowData(1, index)}>
                            Vueltas de {session.name}
                        </Button>
                    </Grid2>
                    {session.raceResult ? (
                        <Grid2 size={3}>
                            <Button color={showData === 2 && showSession === index ? 'warning' : 'error'} variant='contained' fullWidth onClick={() => changeShowData(2, index)}>
                    ...
                    {session.raceResult ? (
                        <Grid2 size={3}>
                            <Button color={showData === 3 && showSession === index ? 'warning' : 'error'} variant='contained' fullWidth  onClick={() => changeShowData(3, index)}>
